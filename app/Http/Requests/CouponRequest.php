    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            if (!$this->isNotNull(request('start_date'))) {
                $validator->errors()->add('start_date', strtolower(trans('all.message.start_date')));
            }

            if (!$this->isNotNull(request('end_date'))) {
                $validator->errors()->add('end_date', strtolower(trans('all.message.end_date')));
            }

            if ($this->isPercentage() && request('discount') > 100) { 
                $validator->errors()->add('discount', strtolower(trans('all.message.percentage_amount')));
            }

            if (!$this->isPercentage() && request('minimum_order') < request('discount')) {
                $validator->errors()->add('minimum_order', strtolower(trans('all.message.minimum_order_amount_discount')));
            }

            if ($this->isNotNull(request('start_date')) && strtotime(request('end_date')) < strtotime(request('start_date'))) {
                $validator->errors()->add('end_date', strtolower(trans('all.message.end_date_older_start_date')));
            }

            if ($this->isNotNull(request('start_date')) && $this->checkToDate()) {
                $validator->errors()->add('end_date', strtolower(trans('all.message.end_date_older_now')));
            }
        });
    }
