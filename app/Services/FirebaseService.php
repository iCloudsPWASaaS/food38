...
class FirebaseService
{
        ...
        try {
            ...
            foreach ($fcmTokens as $fcmToken) {
                try {
                    ...
                    if (str_contains($ex->getMessage(), 'SenderId mismatch') || str_contains($ex->getMessage(), 'UNREGISTERED')) {
                        Log::warning(trans('all.message.invalid_fcm_token_skipped') . $fcmToken);
                        continue;
                    }
                    continue;
                }
    ...
    function getAccessToken()
    {
        ...
        if (isset($parsed_url['path'])) {
            ...
            throw new Exception(trans('all.message.no_file_found_in_the_url'), 422);
        }

        $SCOPES = ['https://www.googleapis.com/auth/cloud-platform'];

        if (!file_exists($this->filePath)) {
            throw new Exception(trans('all.message.service_account_key_file_not_found'), 422);
        }
        ...
        if (isset($token['access_token'])) {
            ...
            throw new Exception(trans('all.message.failed_to_fetch_access_token'), 422);
        }
    }
}