<template>
    ...
    <div id="posMainContent" class="md:w-[calc(100%-340px)] lg:w-[calc(100%-320px)] xl:w-[calc(100%-377px)]" style="position:relative;">
        <div class="flex items-center justify-between mb-4 gap-3">

            <!-- Search Form -->
            <form 
                @submit.prevent="search" 
                class="flex items-center flex-1 h-[38px] leading-[38px] rounded-lg bg-white border border-[#EFF0F6]"
            >
                <input 
                type="text" 
                ...
                class="text-sm text-red-500 fa-regular fa-circle-xmark mr-4"
                ></button>
            ...
            <!-- Hold Orders Button -->
            <button 
                @click.prevent="openHoldOrdersModal" 
                class="flex items-center gap-2 capitalize text-sm font-medium py-2 px-4 rounded-lg text-white bg-[#008BBA] whitespace-nowrap"
            >
        ...
        <div class="my-12" v-else-if="items.length === 0 && props.search.name">
            ...
            =====================================-->
        <div v-if="showAddCustomerPanel" style="position:absolute;top:0;left:0;width:100%;height:100%;background:#fff;z-index:9999;display:flex;flex-direction:column;border-radius:12px;border:1px solid #D9DBE9;overflow:hidden;">
            <div style="display:flex;align-items:center;justify-content:space-between;padding:16px 20px;border-bottom:1px solid #dee2e6;background:#f8f9fa;position:sticky;top:0;">
                <button @click="resetCustomer" style="background:#6c757d;color:#fff;border:none;border-radius:8px;padding:8px 16px;font-size:14px;font-weight:600;cursor:pointer;">
                    ← Back
                </button>
    ...
    =====================================-->
        ...
        <PaymentComponent :props="checkoutProps" :show="showPaymentPanel" @close="showPaymentPanel = false" />
        <!--====================================
    ...
    =====================================-->
        ...
        <div v-if="showHoldOrdersPanel" style="position:absolute;top:0;left:0;width:100%;height:100%;background:#fff;z-index:9999;display:flex;flex-direction:column;border-radius:12px;border:1px solid #D9DBE9;overflow:hidden;">
            <div style="display:flex;align-items:center;justify-content:space-between;padding:16px 20px;border-bottom:1px solid #dee2e6;background:#f8f9fa;position:sticky;top:0;">
                <button @click="closeHoldOrdersModal" style="background:#6c757d;color:#fff;border:none;border-radius:8px;padding:8px 16px;font-size:14px;font-weight:600;cursor:pointer;">
                    ← Back
                </button>
                    ...
                    <table class="table table-hover table-bordered" style="font-size: 15px;">
                        <thead>
                            <tr class="table-light">
                                <th class="align-middle" style="width: 20%; font-size: 13px; font-weight: 700; letter-spacing: 0.5px; text-transform: uppercase; color: #495057; padding: 12px 10px;">Customer</th>
                                <th class="text-center align-middle" style="width: 10%; font-size: 13px; font-weight: 700; letter-spacing: 0.5px; text-transform: uppercase; color: #495057; padding: 12px 10px;">Items</th>
                                <th class="text-right align-middle" style="width: 15%; font-size: 13px; font-weight: 700; letter-spacing: 0.5px; text-transform: uppercase; color: #495057; padding: 12px 10px;">Total</th>
                                <th class="text-center align-middle" style="width: 15%; font-size: 13px; font-weight: 700; letter-spacing: 0.5px; text-transform: uppercase; color: #495057; padding: 12px 10px;">Token</th>
                                <th class="text-center align-middle" style="width: 15%; font-size: 13px; font-weight: 700; letter-spacing: 0.5px; text-transform: uppercase; color: #495057; padding: 12px 10px;">Time</th>
                                <th class="text-center align-middle" style="width: 25%; font-size: 13px; font-weight: 700; letter-spacing: 0.5px; text-transform: uppercase; color: #495057; padding: 12px 10px;">Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr v-for="order in holdOrders" :key="order.id" style="vertical-align: middle;">
                                <td class="align-middle" style="padding: 14px 10px; font-weight: 600; font-size: 15px; color: #212529;">
                                    {{ order.customerName || $t('label.walk_in') }}
                                </td>
                                <td class="text-center align-middle" style="padding: 14px 10px;">
                                    <span style="display: inline-flex; align-items: center; justify-content: center; background: #e9ecef; border-radius: 20px; min-width: 36px; height: 28px; font-weight: 700; font-size: 15px; color: #495057; padding: 0 10px;">
                                        {{ order.items.reduce((sum, item) => sum + item.quantity, 0) }}
                                    </span>
                                </td>
                                <td class="text-right align-middle" style="padding: 14px 10px; font-weight: 700; font-size: 16px; color: #198754;">
                                    {{ currencyFormat(order.subtotal - order.discount, setting.site_digit_after_decimal_point, setting.site_default_currency_symbol, setting.site_currency_position) }}
                                </td>
                                <td class="text-center align-middle" style="padding: 14px 10px;">
                                    <span style="display: inline-block; background: #fff3cd; color: #856404; border: 1px solid #ffc107; border-radius: 6px; padding: 3px 10px; font-weight: 700; font-size: 15px; letter-spacing: 1px;">
                                        {{ order.token || '—' }}
                                    </span>
                                </td>
                                <td class="text-center align-middle" style="padding: 14px 10px; font-size: 14px; color: #6c757d; font-weight: 500;">
                                    {{ formatTime(order.timestamp) }}
                                </td>
                                <td class="text-center align-middle" style="padding: 14px 10px;">
                                    <div class="d-flex justify-content-center gap-2">
                                        <button
                                            @click.prevent="loadHoldOrder(order.id)"
                                            style="display: inline-flex; align-items: center; gap: 6px; background: #0d6efd; color: #fff; border: none; border-radius: 8px; padding: 8px 16px; font-size: 14px; font-weight: 600; cursor: pointer; transition: background 0.2s, transform 0.1s; box-shadow: 0 2px 6px rgba(13,110,253,0.3);"
                                            onmouseover="this.style.background='#0b5ed7'; this.style.transform='translateY(-1px)';"
...
export default {
    ...
    methods: {
        ...
        resetCart: function () {
            this.$store.dispatch('posCart/resetCart').then(res => { //extra
        ...
        addCustomers: function () {
            ...
            this.showAddCustomerPanel = true;
        },
        resetCustomer: function () {
            //appService.modalHide("#addCustomer");
            this.showAddCustomerPanel = false;
        ...
        generateToken: function() {
            // Generate a random 4-digit number for the token
            const randomToken = Math.floor(1000 + Math.random() * 9000);
            this.checkoutProps.form.token = randomToken.toString();
        },
    },
    watch: {
